...
function processJson(myJson) {
    ...
    var maximum = Math.max.apply(Math, myJson);
    var minimum = Math.min.apply(Math, myJson);
    console.log(maximum);
    console.log(minimum);
    var diff = (maximum - minimum) / 18.0;
    if(maximum == minimum) {
        diff = 1;
    }
    ...
    if (myJson && myJson.length) {
        for(i=0;i<myJson.length;i++) {
            distanceVal = myJson[i]["distance"];
            distanceVal = 36 - (distanceVal * diff);
            jsonObj = {
                type: 'type1',
                severity: 'ordinary',
                coords: [103.7, distanceVal], 
            };
            jsonData.push(jsonObj);
        }
    }
    console.log(jsonData);
    radar.addRiskPoints(jsonData);
}

const myRequest = new Request("http://127.0.0.1:8000/radarAPI/");
fetch(myRequest)
